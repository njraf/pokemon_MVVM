...
QSharedPointer<AttackMove> AttackMoveDao::getAttackByID(int id, int effectID) {
    ...
    if (!model.select()) {
        qDebug() << "ERROR: getAttackByID()::select() failed" << model.lastError().text();
    }
...
QSharedPointer<AttackMove> AttackMoveDao::getAttackByName(QString name, int effectID) {
    ...
    if (!model.select()) {
        qDebug() << "ERROR: getAttackByName()::select() failed" << model.lastError().text() << "No attack named" << name;
    }
                ...
                if ((col == 0) || ((col > 1) && (col < 5))) { // int data
                    ...
                    if (AttackMove::strToUnderlying.contains(strData)) {
                        ...
                        if (!table.setData(table.index(row, col), intData)) {
                            qDebug() << "ERROR: Could not set category data" << table.lastError().text();
                        }
